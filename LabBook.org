...
** STARTED Principais trabalhos relacionados - Artigo do Luis Felipe
- State "STARTED"    from              [2016-08-03 Qua 00:34]
|------------+-----------------------+----------------------------------+---------------------------------------------+----------------------+----------------------------------+-------------------------------------+----------------------|
| Autores    | Titulo                | Objetivo                         | Metodologia                                 | Benchmark,           | Resultados                       | Observaçao                          | Diferença            |
|            |                       |                                  |                                             | Ferramenta ou        |                                  |                                     | do nosso trabalho    |
|            |                       |                                  |                                             | Tecnologia           |                                  |                                     |                      |
|------------+-----------------------+----------------------------------+---------------------------------------------+----------------------+----------------------------------+-------------------------------------+----------------------|
| Laurenzano |                       |                                  |                                             |                      | - Os principais                  | - O autor nao chega a definir,      | - Foco em aplicaçoes |
| et al.     | =Reducing energy usage= | - Abordagem que permite          | - Instrumentaçao realizada                  | - Benchmark          | resultados adquiridos            | mas menciona que uma aplicaçao      | paralelas (OpenMP)   |
|            | =with memory and=       | selecionar *por laço* a melhor     | atraves da utilizaçao da                    | proprio: =pcubed=      | pelos autores foi a              | =memory-bound= depende primeiramente  | - Regiao paralelas   |
|            | =computation-aware=     | frequencia de clock.             | ferramenta PEBIL toolkit                    | (PMaC's Performance  | economia de cerca                | da proximidade, tamanho e do        | dentro da abordagem  |
|            | =dynamic frequency=     | O criterio de frequencia mais    | - Uso da biblioteca cpu-freq                | and Power benchmark) | de 7,6% de energia para          | Speedup disponivel com as operaçoes | fork-join            |
|            | =scaling=               | adequada esta entre desempenho e | para alterar a frequencia                   |                      | a primeira plataforma de         | sob a memoria.                      |                      |
|            |                       | consumo de energia               | do processador                              |                      | execuçao (Intel Xeon E5530)      |                                     |                      |
|            |                       |                                  | - Experimento: os paramentos                |                      | e para a segunda plataforma      |                                     |                      |
|            |                       |                                  | levados em consideraçao para                |                      | (AMD Opteron 8380) cerca         |                                     |                      |
|            |                       |                                  | aplicaçao sao: taxa de hit                  |                      | de 10,6%.                        |                                     |                      |
|            |                       |                                  | dos diferentes niveis de cache,             |                      | - Vale lembrar, que esses        |                                     |                      |
|            |                       |                                  | operaçoes de ponto-flutuante (FP) para      |                      | ganhos usando DVFS puramente     |                                     |                      |
|            |                       |                                  | operaçoes de memoria e a media              |                      | de maneira consciente (a partir  |                                     |                      |
|            |                       |                                  | da computaçao de operaçoes em FP            |                      | do conhecimento do comportamento |                                     |                      |
|            |                       |                                  | e inteiro.                                  |                      | da aplicaçao)                    |                                     |                      |
|            |                       |                                  | - A plataforma de execuçao escolhida        |                      |                                  |                                     |                      |
|            |                       |                                  | por eles *permite a alteraçao da frequencia*  |                      |                                  |                                     |                      |
|            |                       |                                  | *independentemente de cada core* (frequencias |                      |                                  |                                     |                      |
|            |                       |                                  | avaliadas 1.60, 1.73, 1.86, 2, 2.13, 2.26,  |                      |                                  |                                     |                      |
|            |                       |                                  | 2.39 ou 2.40 GHz)                           |                      |                                  |                                     |                      |
|------------+-----------------------+----------------------------------+---------------------------------------------+----------------------+----------------------------------+-------------------------------------+----------------------|
...
** Definiçãoo do assunto da dissertação
...
** Instalaçao do Scorep

- Um dos pre-requisitos de instalaÃ§ao do Scorep e a instalaÃ§ao do
  =Qt_app=.
...
vai ser criado em python, porque como sÃ£o muitos dado
...
- Vou tentar novamente com as permissÃµes alteradas
...
para apenas uma thread e uma Ãºnica mÃ©trica.
...
* [14:13:14; 19.07.2016] Adicionar coluna de tempo ao CSV

#+begin_src sh :results output :exports both
cd benchmarks/MM/exec_continuos_par80/
#+end_src

#+RESULTS:
: graph.pdf
: threadAnalyseOutput.csv
