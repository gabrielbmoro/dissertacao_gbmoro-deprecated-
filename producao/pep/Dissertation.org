...
para um programação paralelo de acordo com seu comportamento, o qual
pode ser mais memory-bound ou CPU-bound para algumas das regiões do
programa. Com essas regiões identificadas, o trabalho utilizará a
técnica DVFS (/Dynamic Voltage and Frequency Scaling/) para obter a
frequência adequada para cada região. De forma mais detalhada, a
proposta do trabalho pode ser dividida em três etapas, as quais são
