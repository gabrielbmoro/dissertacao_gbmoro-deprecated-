...
diferentes configurações de frequência para o processamento (uso de DVFS), como também a nível de software, 
utilizando técnicas de balanceamento de carga, a fim de melhor distribuir a
...
cache misses, fazendo com que essas aplicações sejam limitadas pelo acesso a memória em alguns trechos de execução. 
Outras aplicações, realizam mais processamento e acessam a memória menos vezes, como é o caso das aplicações mais 
CPU-bound que possuem um
IPC (
#+BEGIN_LaTeX
...
pois é efetuado o mesmo cálculo sobre diferentes dados, dessa forma, o registro da informação é realizado 
e buscado antes do novo cálculo \cite{Morowsppd}. Portanto,
...
Esse capítulo apresenta os conceitos básicos que estão relacionados com o trabalho, dentre eles, 
comportamento de aplicações paralelas, modelos de programação, consumo de energia de aplicações, 
entre outros.
...
``\textit{omp task}''
#+END_LaTeX
 permite realizar uma concorrência daquela tarefa dentre as
