...
\citeonline{Poellabauer2005} demonstram uma técnica chamada 
#+BEGIN_LaTeX
...
#+END_LaTeX
). A partir dessa equação é possível analisar a porcentagem de misses 
nas instruções executadas pela aplicação como um todo.
