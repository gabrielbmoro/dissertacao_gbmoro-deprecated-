...
diferentes configurações de frequência para o processamento (uso de
DVFS), como também a nível de software, utilizando técnicas de
balanceamento de carga, a fim de melhor distribuir a computação entre
os recursos de processamento.

#+Latex: \review{Definir o que é DVFS! Dynamic Voltage and Frequency Scaling. Traduzindo: Mudança dinâmica de voltagem e frequência.}

Uma aplicação possui aspectos peculiares, os quais definem seu tipo,
modelo e demais características de processamento. 
#+Latex: \review{Não entendi o que quis dizer por "aspectos peculiares".}
Além dessas
características influenciarem o comportamento de uma aplicação, a
plataforma de execução influencia diretamente no seu
comportamento. Aplicações \review{do tipo} memory-bound é 
#+Latex: \review{são!  (favor revisar a concordância de todas as tuas frases. Isso é o mínimo de qualidade a ser exigido de qualquer texto.}
uma classe de aplicações
que possui alta taxa de cache misses, fazendo com que essas
aplicações sejam limitadas pelo acesso a memória em alguns trechos de
execução. Outras aplicações, 
#+Latex: \review{Aqui tu separas o sujeito do verbo com a vírgula. Isso não existe em português. Daqui para frente vou me limitar a comentários técnicos e científicos. Deixo para tu resolver teus problemas de escrita em português. Veja marcações "!!".}
realizam mais processamento e acessam a
memória menos vezes, como é o caso das aplicações mais CPU-bound que
possuem um 
IPC (/Instructions Per Cycle/) maior.
#+Latex: \review{Traduzir para português (veja que é instruções e não instrução): instruções por ciclo}
...
cálculo sobre diferentes dados, dessa forma, o registro da informação
é realizado e buscado antes do novo cálculo
\cite{Morowsppd}. 
#+Latex: \review{Não ficou claro para mim o que tu quis dizer com "realizado e buscado"}
Portanto, em uma aplicação é possível ter diferentes
comportamentos em uma granularidade mais fina, compreendendo o
comportamento das diferentes regiões da aplicação. O objetivo do
trabalho é detectar as regiões memory-bound de aplicações paralelas de
maneira automática, em uma granularidade refinada, diferente das
abordagens investigadas,
#+Latex: \review{Quais abordagens investigadas?}
 de maneira que possa ser realizado ajustes de
frequência de processador utilizando a técnica 
DVFS (/Dynamic Voltage and Frequency Scaling/)
 para reduzir o consumo de energia nas fases mais memory-bound da
aplicação.
#+Latex: \review{memory-bound poderia ser traduzido já que tu usas de maneira recorrente.}

O presente trabalho está organizado da seguinte maneira: no capítulo 2
será \review{!!} apresentado os conceitos básicos utilizados em nossa abordagem;
...
e no capítulo 5 é apresentado \review{!!} as conclusões do trabalho, como também
os próximos passos da pesquisa e cronograma de atividades que serão
realizadas no decorrer do mestrado.

#+Latex: \review{Sinto que faltou falar dos trabalhos relacionados, pelo menos um parágrafo dizendo o que já foi feito na área e o que tu propõem de diferente seria ideal. Isso depois é detalhado no estado da arte, mas acho bom tu colocares aqui também como forma de motivar do ponto de vista científico o teu trabalho.}
...
\review{Tu podes colocar em itálico simplesmente usando /barras/, veja}
\to /barras/
...
\textit{fork}
#+END_LaTeX
), após essa divisão, geralmente ocorre um processamento
...
OpenMP) \cite{pacheco2011introduction}
#+Latex: \review{Do ponto de vista conceitual, Pthreads e OpenMP (somente for) são idênticos. Então considero errado o que tu escreveu nesta frase.} 
. Algoritmos que utilizam a
...
#+latex: \review{Tu fala de três, mas na figura está escrito só de dois.}.
...
obter um novo resultado para determinado elemento da matriz
\review{que matriz?}, levando
...
compor uma borda da matriz, faltará \review{!!} alguns vizinhos para
...
#+latex: \review{Com assim duplicação?} 
ou o preenchimento com um valor constante
#+Latex: \review{Isso não faz o resultado ser errado?}. 
...
#+Latex: \review{O que isso difere de fork-join?}
...
#+BEGIN_LaTeX
``\textit{pragma omp parallel for}''
#+END_LaTeX
 sinaliza o momento em que ocorrerá o disparo das threads, aonde
...
``\textit{omp task}''
#+END_LaTeX
 permite realizar uma concorrência daquela tarefa entre as
...
#+BEGIN_LaTeX
``\textit{pragma omp taskwait}''
#+END_LaTeX
, fazendo com que as threads que terminaram as
...
base \review{Frase grande!}. Outra abordagem de balanceamento de carga para uma aplicação
...
fazendo com que esse tipo de técnica possua um 
#+latex: \review{sobrecarga é tradução de overhead}
#+BEGIN_LaTeX
\textit{overhead}
#+END_LaTeX
 baixo, quando
...
realizará o seu trabalho.  \review{frase longa}
...
O conjunto de Mandelbrot é um exemplo de algoritmo onde
#+latex: \review{é necessário (ocorre somente se alguém o fizer)}
ocorre um
balanceamento dinâmico. Esse algoritmo consiste no cálculo de
...
\cite{chandra2001parallel}. \review{frase grande}
...
diferente, porque realizam processamentos distintos 
#+latex: \review{Me parece óbvio demais.}
. O algoritmo de busca em grafos, por exemplo, possui um comportamento
mais memory-bound do que o algoritmo de multiplicação de vetores, pois
...
principal várias vezes 
#+latex: \review{Seria bom colocar uma evidência para suportar teu argumento. Uma citação ou um gráfico que mostre isso.}
. Diferente do que ocorre no algoritmo de
multiplicação de vetores, aonde os dados estão mais bem localizados 
#+latex: \review{o que é estar "mais bem localizado"?}
 na
...
aplicações non-memory-bound
#+latex: \review{Não memory-bound, ou simplesmente CPU-bound (se não for IO-bound).}
 não ocorre o aumento de desempenho a medida 
...
quantidade de acessos aos níveis de cache, memória principal, a taxa
de hit  
#+latex: \review{hit seria acerto.}
nas memórias, a quantidade de instruções executadas e assim por diante.

Além da taxa de misses 
#+latex: \review{traduzir misses?}
para o segundo nível de cache para definir o 
...
o seu desempenho, se isso acontecer podemos classificar a aplicação como non-memory-bound 
ou CPU-bound \cite{jesshope2006advances}.
...
real) ou a partir de modelos (estimativa teórica).
#+latex: \review{Não se pode medir através de modelos, jamais! O máximo que tu podes fazer é estimar com modelos.}
 Algumas ferramentas
...
processador, esse framework possui dois módulos, o 
#+BEGIN_LaTeX
\textit{Governor}
#+END_LaTeX
...
#+Latex: \review{Dizer também que frequentemente trabalhos que tentam economizar o consumo energético de aplicações o fazem comparando-se com os resultados de consumo obtidos com esses governors.}

#+Latex: \review{Remover esses blocos latex, substituir por /texto/ (impossível de ler no emacs da forma como está).}
...
#+Latex: \review{Citar trabalho que suporte essa última afirmação (estou curioso).}
...
#+BEGIN_LaTeX
 \textit{Basic blocks}
#+END_LaTeX
#+Latex: \review{blocos básicos é palavra reservada na computação: se refere aos blocos básicos definidos pelo compilador. Evitar usar esse termo se possível portanto, a menos que te refira a mesma coisa, o que não parece ser o caso.}
...
#+latex: \review{Senti falta de um capítulo antes deste onde tu detalhas em duas páginas qual é a tua proposta. Nesse capítulo basicamente tu escreverás o que te mostrei com aquela figura que tirei a foto.}

#+latex: \review{Ao entrar direto nos experimentos preliminares, claramente falta o capítulo de proposta, afinal isto é o texto de um projeto.}
...
50, 100, 150 e 200 milesegundos \review{!!}. Também, foram realizadas execuções com o uso
