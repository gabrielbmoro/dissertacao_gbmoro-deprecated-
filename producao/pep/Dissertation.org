...
modelo e demais características de processamento. Além dessas
características influenciarem o comportamento de uma aplicação, a
plataforma de execução influencia diretamente no seu
comportamento. Aplicações memory-bound é uma classe de aplicações
...
execução. Outras aplicações, realizam mais processamento e acessam a
...
\cite{Morowsppd}. Portanto, em uma aplicação é possível ter diferentes
...
abordagens investigadas, de maneira que possa ser realizado ajustes de
...
compor uma borda da matriz, faltará alguns vizinhos para
