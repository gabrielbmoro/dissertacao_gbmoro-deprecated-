...
tstart = min(df$Start);
tend = max(df$End);
gantt <- gantt_plot(df, 1, tstart, tend) + theme(legend.position="top", axis.text.x=element_blank(), axis.title.x=element_blank());
l2_icm <- hw_counter_plot (df, 1, "PAPI_L2_ICM", tstart, tend) + theme(axis.text.x=element_blank(), axis.title.x=element_blank());
l2_dcm <- hw_counter_plot (df, 1, "PAPI_L2_DCM", tstart, tend) + theme(axis.text.x=element_blank(), axis.title.x=element_blank());
l2_tca <- hw_counter_plot (df, 1, "PAPI_L2_TCA", tstart, tend);
...
Os eixos X não estão sincronizados, mas deveriam.
