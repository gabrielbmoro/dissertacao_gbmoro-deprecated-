...
* 2017-02-20 Próximos passos
De nada adianta olhar para os contadores se não detectar as regiões
paralelas no rastro. Veja o uso de eztrace (seção anterior) para
rastrear uma aplicação OpenMP pequena; sinto que as regiões paralelas
são corretamente capturadas. Faltaria então registrar os contadores e
fazer o experimento com as três aplicações principais que tu vens
