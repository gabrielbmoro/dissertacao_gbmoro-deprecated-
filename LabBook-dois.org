...
tidsdf = data.frame(Thread=tids, ntids = 1:length(tids));
df2 <- merge (df1, tidsdf, by.x="Thread", by.y="Thread");
df2 <- df2 %>% select(-Thread) %>% rename(Thread=ntids) %>% group_by(Thread,Region) %>% as.data.frame();
...
-Pop_PAPI_L2_TCM,-Pop_PAPI_L2_TCA,-Pop_ru_utime, -Pop_ru_stime) %>%
	mutate(
		MISSES_L2 = PAPI_L2_TCM / PAPI_L2_TCA,
            IPC = PAPI_TOT_INS / PAPI_REF_CYC) %>%  
    select(-PAPI_TOT_INS,-PAPI_REF_CYC,-PAPI_L2_TCM, -PAPI_L2_TCA, -ru_utime, -ru_stime)

#Separando Regiões e Linhas de Código
df3$line <- as.numeric(gsub("*[a-z]*|@|:|!\\$|\\.","",df3$Region))
df3$Region <- gsub("*[0-9]*|:|@[a-z]*.[a-z]|!\\$","",df3$Region)
df3$Region <- gsub("\\ ","_",df3$Region)

#agrupando os contadores por região
df4 <- select(df3, Start, End, Duration, -Imbrication, Region, line, Thread, MISSES_L2,IPC);
df4 <- df4 %>% group_by(Thread, Region, line) %>% summarise(mean_misses_l2 = mean(MISSES_L2),
 mean_ipc=mean(IPC)) %>% as.data.frame();

#agora vamos filtrar misses igual ou acima de 30% e ipc menor ou igual a 10
df5 <- df4 %>% filter(!(mean_misses_l2 < 0.3 | mean_ipc > 10));

head(df5);
#+end_src

#+RESULTS:
:   Thread                Region line mean_misses_l2 mean_ipc
: 1      1 omp_implicit_barrier_  221      0.3983740 9.548906
: 2      1 omp_implicit_barrier_  549      0.5837766 9.731087
: 3      1 omp_implicit_barrier_  657      0.6216951 9.758888
: 4      1 omp_implicit_barrier_  840      0.3246753 9.503844
: 5      1 omp_implicit_barrier_  868      0.5011287 9.302465
: 6      2               omp_do_  836      0.3078551 7.124884

- Professor, está correto a maneira com que agrupei as regiões? 
Pensei da seguinte forma, a prioridade do agrupamento deve ser por
  Threads, visto que em um mesmo momento são coletadas as métricas de
  misses e ipc para diferentes threads. Depois de separar os dados por
  Threads, agrupei como prioridade secundária por Região e linha de
  código. Pensei que a melhor maneira seria utilizar uma média para as
  métricas serem agrupadas. Após isso, estabeleci uma faixa de
  valores, aonde considerei misses na l2 a cima ou igual 30% e IPC
  menor ou igual a 10. Nesse primeiro momento pensei nesses valores
  como limitadores, não sei se esses são os mais indicados, para
  analisar corretamente o IPC ideal, deveria ser levado em
  consideração aspectos arquiteturais da plataforma. O misses na l2
  sabemos, por experimentos anteriores que >= 30% é um limite
  interessante para considerar uma região memory-bound. Será que uma
  equação utilizando essas duas métricas poderia ser o mais indicado
  para considerar essa uma região candidata a ser executada em uma
  frequência baixa?
