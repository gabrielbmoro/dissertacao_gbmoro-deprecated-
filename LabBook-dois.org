...
** WAITING [12:34:41; 07.02.2017] Analisando a aplicação CG com R  :Gabriel:

- State "WAITING"    from "STARTED"    [2017-02-12 Dom 17:37]
...
exportByThreadAndRegion <- function(thread,region) {
	df2 <- df1 %>% filter(Thread==thread)
	df3 <- df2 %>% filter(str_detect(Region, region))
	...
	vectorTmp <- NULL
	m_final <- matrix(ncol = 7,nrow=sizeOfLines-1)
	index_final=1
	...
	while(i < nrow) {
		...
		if (i + 1 < nrow) {
			...
			while(count < sizeOfLines) { 
                            ...
                            if(lines[count] == df3[i,j]) {
           	                	initL <- df3[i,j]
					duration <- df3[i,4] + duration
					papi_tot_inst <- df3[i,5] + papi_tot_inst
					papi_ref_cyc <- df3[i,6] + papi_ref_cyc
					papi_l2_tcm <- df3[i,7] + papi_l2_tcm
			        	papi_l2_tca <- df3[i,8] + papi_l2_tca
					loc <- count
				}
			
				else if(lines[count] == df3[i+1,j]) {
    	                        endL <- df3[i+1,j]
           		                duration <- df3[i+1,4] + duration
					papi_tot_inst <- df3[i+1,5] + papi_tot_inst
					papi_ref_cyc <- df3[i+1,6] + papi_ref_cyc
					papi_l2_tcm <- df3[i+1,7] + papi_l2_tcm
				        papi_l2_tca <- df3[i+1,8] + papi_l2_tca
                                	loc <- count
				}
			...
			m_final[index_final,1] <- initL
			m_final[index_final,2] <- endL
			m_final[index_final,3] <- duration
	        	m_final[index_final,4] <- papi_tot_inst
			m_final[index_final,5] <- papi_ref_cyc
			m_final[index_final,6] <- papi_l2_tcm
			m_final[index_final,7] <- papi_l2_tca
			index_final <- index_final + 1
		}
		i <- i + 1
    }

	df_final <- data.frame(m_final)
	df_final <- df_final %>% filter(is.na(X3)==FALSE) %>% as.data.frame();
	df_final$X8 <- thread
    df_final$V9 <- region
	df_final <- unique(df_final);
	return (df_final)
}
exportByThreadAndRegion(thread,region);
#+end_src
...
5       18889
6       22032
    X1  X2        X3       X4      X5     X6     X7 X8       V9
1  196 253  0.108782    13112    1642    577   1714  1 parallel
2  253 266  0.026727     7756     463     98    415  1 parallel
3  266 281  0.030286     7796     462     83    442  1 parallel
4  281 323  1.164771   268202   18675   3986  19059  1 parallel
5  323 344  3.461855   527130   41495  12080  40921  1 parallel
6  344 488  4.711526   804570   50098  10639  42722  1 parallel
7  488 522 98.050641 20897845 1031792 121797 637909  1 parallel
8  522 646 98.270981 20894942 1035213 123216 635593  1 parallel
9  646 736  2.646309   542624   31141   4167  18968  1 parallel
10 736 836  0.040402     7605     853    286    995  1 parallel
11 836 862  0.036196     7643     625    172   1078  1 parallel
#+end_example
...
- Acredito que essa função pode ser melhorada e refinada, fiz essa
  primeira versão. Pretendo melhorar bastante, a partir dos estudos em
  R que estou fazendo. Após refinar essa função essa poderá ser
  chamada de acordo com o número de threads e regiões possíveis.


- Como já consigo agregar os contadores de hardware por thread e por
  região, vou utilizar a função criada. A partir dela é possível obter
  um frame de dados com diferentes métodos de uma thread, logo será
  necessário chamar essa função N vezes Z, onde N é as variações de
  métodos de código e Z representa o número de threads.

#+begin_src R :results output :session *R* :exports both
regions <- unique(df1$Region)
threads <- unique(df1$Thread)

resultFrame <- NULL

for(k in regions){
	for(l in threads) {
		frameTmp <- exportByThreadAndRegion(l, k)
		resultFrame <- rbind(resultFrame,frameTmp)
	}
}

resultFrame <-  resultFrame %>% dplyr::rename(startL=X1,endL=X2,duration=X3,tot_inst=X4,tot_cyc=X5,l2_tcm=X6,l2_tca=X7,thread=X8,parallelR=V9)
resultFrame <- resultFrame %>% dplyr::mutate(ipc = tot_inst / tot_cyc, l2misses_rate = l2_tcm / l2_tca) %>% select(-tot_inst,tot_cyc,-l2_tcm,-l2_tca)
head(resultFrame)
#+end_src

#+RESULTS:
:   startL endL duration tot_cyc thread parallelR       ipc l2misses_rate
: 1    196  253 0.118973    1527     40 parallel_  8.550753    0.32680196
: 2    253  266 0.026003     416     40 parallel_ 18.548077    0.11471322
: 3    266  281 0.024472     389     40 parallel_ 19.938303    0.13020833
: 4    281  323 1.125844   14326     40 parallel_ 18.812579    0.14294621
: 5    323  344 2.251016   26512     40 parallel_ 19.983140    0.11230437
: 6    344  488 3.672486   41262     40 parallel_ 19.554457    0.09723976

- Aguardando a opinião do professor Lucas sobre essa agregação de
  dados que fiz.

* [20:40:10; 26.02.2017] Instalando o Extrae                        :Gabriel:

- Instalando Extrae para realizar novos experimentos.

#+begin_src sh :results output :exports both
pathOfInstallation=/home/gabrielbmoro/Programas/

cd $pathOfInstallation
#wget https://ftp.tools.bsc.es/tracking/tracking-2.6.5-linux_x86_64.tar.gz

#tar xvf tracking-2.6.5-linux-x86-64

#mv tracking-2.6.5-linux-x86-64 extrae

#rm -rf *.tar.gz

#sed -i '1iexport EXTRAE_HOME='"$pathOfInstallation"'extrae' ~/.
...
#+RESULTS:
#+begin_example
EXTRAE_HOME points to /home/gabrielbmoro/Programas/extrae and the directory exists .. OK
Loaded specs for Extrae from /home/gabrielbmoro/Programas/extrae/etc/extrae-vars.sh

Extrae was configured with:
$ ../configure --prefix=/apps/BSCTOOLS/extrae/3.4.3/openmpi+libgomp4.9 --enable-posix-clock --with-mpi=/apps/OPENMPI/1.8.1-mellanox --with-load-balancing=/apps/PM/dlb/latest/openmpi --with-papi=/apps/BSCTOOLS/deps/papi/5.5.1 --with-dyninst=/apps/BSCTOOLS/deps/dyninst/9.3.0 --with-dwarf=/apps/BSCTOOLS/deps/libdwarf/20160613 --with-elf=/apps/BSCTOOLS/deps/libelf/0.148 --with-unwind=/apps/BSCTOOLS/deps/libunwind/1.2-rc1 --with-boost=/apps/BSCTOOLS/deps/boost/1.61.0 --with-binutils=/apps/BSCTOOLS/deps/binutils/2.24 --with-xml-prefix=/apps/BSCTOOLS/deps/libxml/2.9.4 --enable-online --with-synapse=/apps/BSCTOOLS/synapse/2.0.0 --with-spectral=/apps/BSCTOOLS/spectral/3.4.0 --with-clustering=/apps/BSCTOOLS/clusteringsuite/2.6.6 --enable-sampling
...
MPI_HOME points to /apps/OPENMPI/1.8.1-mellanox and the directory does NOT exist .. FAILED
LIBXML2_HOME points to /apps/BSCTOOLS/deps/libxml/2.9.4 and the directory does NOT exist .. FAILED
PAPI_HOME points to /apps/BSCTOOLS/deps/papi/5.5.1 and the directory does NOT exist .. FAILED
DYNINST_HOME points to /apps/BSCTOOLS/deps/dyninst/9.3.0 and the directory does NOT exist .. FAILED
DWARF_HOME points to /apps/BSCTOOLS/deps/libdwarf/20160613 and the directory does NOT exist .. FAILED
UNWIND_HOME points to /apps/BSCTOOLS/deps/libunwind/1.2-rc1 and the directory does NOT exist .. FAILED
LIBERTY_HOME points to /apps/BSCTOOLS/deps/binutils/2.24 and the directory does NOT exist .. FAILED
BFD_HOME points to /apps/BSCTOOLS/deps/binutils/2.24 and the directory does NOT exist .. FAILED

IO instrumentation is: ENABLED
Dynamic memory instrumentation is: ENABLED
