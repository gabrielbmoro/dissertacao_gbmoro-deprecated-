...
#+begin_src sh :results output :exports both :tangle scripts/instalaExtrae.bash
#!/bin/bash

pathOfDownload="$HOME/Downloads/"
pathOfInstallation="$HOME/Programs/"

function instalaPrerequisitos() {
	...
	tar vxf libunwind-1.2.tar.gz
	./libunwind-1.2/configure
	cd libunwind
	make
	sudo make install
	rm -rf *libunwind-1.2*
...
function instalaExtrae() {

    
    cd $pathOfDownload
 

    #instalação do extrae

    cd $pathOfInstallation
    wget https://ftp.tools.bsc.es/extrae/extrae-3.4.3-src.tar.bz2
    tar xvjf extrae-3.4.3-src.tar.bz2
    rm -rf *.bz2

    PREFIX="$HOME/Programas/extrae-3.4.3/"

    cd $PREFIX

    ./configure --prefix=$PREFIX --with-bfd=$PREFIX \
		--enable-openmp \
		--with-libgomp=auto \
		--without-mpi \
		--enable-instrument-dynamic-memory \
		--with-unwind=/usr/local/bin \
		--with-unwind-libs=/usr/local/lib \
		--with-unwind-headers=/usr/local/include \
		--without-dwarf \
		--without-dyninst \
		--without-elf \
		--with-papi-headers=/usr/local/include \
		--with-papi-libs=/usr/local/lib \
		--with-papi=/usr/local/bin \
		--enable-sampling \
		--without-binutils 

    make

    make install 2> mierr

}


instalaPrerequisitos
...
cd $HOME/Programas/extrae-3.4.3
make check | grep -i FAIL
#+end_src
...
- Definindo variável de ambiente =EXTRAE_HOME=

#+begin_src sh :results output :exports both
cd $HOME/Programs/extrae-3.4.3/
sed -i "1iexport EXTRAE_HOME=$(pwd)" ~/.bashrc
#+end_src

- A cada utilização, deverá ser disparada a seguinte linha de comando
  para configurar para a nova seção a variável de ambiente definida:

#+begin_src sh :results output :exports both
source ${EXTRAE_HOME}/etc/extrae.sh

export LD_PRELOAD=${EXTRAE_HOME}/lib/libomptrace #como usamos openmp
#+end_src

- Estou lendo a documentação, nunca tinha utilizado o Extrae antes,
  mas a princípio, a ferramenta utiliza uma variável de ambiente
  chamada =EXTRAE_CONFIG_FILE=. Acredito que a partir dela seja possível
  especificar um arquivo xml, com as configurações da instrumentação,
  como o exemplo encontrado em:

#+begin_src sh :session foo :results output :exports both 
cd $HOME/Programas/extrae-3.4.3/example/LINUX/OMP/
cat extrae.xml
#+end_src
...
TRACE</trace-prefix>
5</size>
/scratch</temporal-directory>
/gpfs/scratch/bsc41/bsc41273</final-directory>
...
/gpfs/scratch/bsc41/bsc41273/control</file>
</global-ops>
