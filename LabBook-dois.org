...
- State "WAITING"    from "TODO"       [2017-03-09 Qui 21:06]
- State "TODO"       from "WAITING"    [2017-03-09 Qui 21:06]
...
- É necessário copiar os rastros que estão na =hype2=, no seguinte path:
  */home/gabrielbmoro/rastros/exp7-NAS-cg_mg_ft-40threads.tar.gz*. Após
  isso, os seguintes passos podem ser realizados.
...
df2 <- df1 %>% melt(c("Start","End","Duration","Imbrication","Region","Thread","ru_utime","ru_stime","line"))
head(df2)
#+end_src
...
5  221.24101  221.24811   0.007102           0         parallel_     40
6 8024.99327 8025.06138   0.068103           0         parallel_     40
  ru_utime ru_stime line     variable   value
1        0        0  736 PAPI_TOT_INS    2040
2        0        0  736 PAPI_TOT_INS   18855
3     8000        0  739 PAPI_TOT_INS 2102041
4        0        0  736 PAPI_TOT_INS    1771
5        0        0  736 PAPI_TOT_INS    1763
6        0        0  836 PAPI_TOT_INS    2013
#+end_example


- Problemas do data.frame:
  a) As métricas devem ser agrupadas em um campo variável para
     facilitar a agregação dos dados (está pronto com o uso do melt)
  b) Temos que estabelecer uma faixa padrão de intervalos na marcação
     do Tempo inicial e no Tempo final, por exemplo: vamos imaginar
     duas threads, a primeira thread de id 2, a sua primeira coleta de
     contadores de hardware ocorre no tempo 11.12 segundos. Já, para
     uma thread k + 1, a primeira coleta de tempo ocorre em 11.40. Uma
     das maneiras que vejo resolver seria excluir a coluna que
     representa o tempo final, manter a duração e arredondar para um
     inteiro 

#+begin_src R :results output :session *R* :exports both
df3 <- df2 %>% mutate(Start = round(Start, digits=0)) %>% select(Start, -End, Duration,
 -Imbrication, Region, Thread, -ru_utime, -ru_stime, line, variable, value)

df_unique <- df3 %>% select(Start, Region, line) %>% unique()

df4 <- df3 %>% select(-Thread,-Start) %>% group_by(Region, line, variable) %>%
    summarize(time=sum(Duration), value=sum(value)) %>%
    as.data.frame()
...
#+RESULTS:
#+begin_example
  Region line     variable      time     value
1    do_  197 PAPI_TOT_INS 217.19859 124429166
2    do_  197 PAPI_REF_CYC 217.19859   4623788
3    do_  197  PAPI_L2_TCM 217.19859    150765
4    do_  197  PAPI_L2_TCA 217.19859    100134
5    do_  208 PAPI_TOT_INS  54.41479    643630
6    do_  208 PAPI_REF_CYC  54.41479     51074
  Start            Region line
1    12         parallel_  736
2    12               do_  736
3    12 implicit_barrier_  739
4   221               do_  736
5   221         parallel_  736
6  8025         parallel_  836
#+end_example

- Próximo passo agora é agregar os dados pelas diferentes threads, por
  exemplo, nesse momento, o data.frame está com várias duplicações,
  pois duplicamos para ter os contadores como variáveis. O ideal seria
  agrupar de maneira coesa o que temos no df4


- Aqui em baixo estou iniciando algo, mas to achando
  estranho. Professor considera dessa linha pra cima, a seguir to
  trabalhando.

#+begin_src R :results output :session *R* :exports both

dfunique_size= nrow(df_unique)
df4_size = nrow(df4)

m_final <- matrix(ncol=6, nrow=df4_size-1)
index_mfinal <- 1
...
while(i < df4_size) {    
    while(j < dfunique_size) {
        
        if(df4[i, 1] == df_unique[j,2] & df4[i,2] == df_unique[j,3]) {
#            m_final[index_mfinal,1] <- df_unique[j,1]
#            m_final[index_mfinal,2] <- df4[i,4]
#            m_final[index_mfinal,3] <- df4[i,1]
#            m_final[index_mfinal,4] <- df4[i,2]
#            m_final[index_mfinal,5] <- df4[i,3]
#            m_final[index_mfinal,6] <- df4[i,5]
#            index_mfinal <- index_mfinal + 1
        }
        j <- j + 1
    }
    j <- 1
...
#+RESULTS:
: Error in `[<-`(`*tmp*`, index_mfinal, 1, value = 27363) (from #5) : 
